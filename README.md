...
- **External hardware scanners (wired or wireless) are also supported**, hands-free, on the staff scan panel, alongside the phone camera. USB and Bluetooth QR/barcode scanners work as keyboard-wedge devices — they type the scanned code into whatever's focused, then send Enter — so the code-entry field on the scan panel stays focused automatically and submits on Enter. No pairing/setup in the app itself is needed beyond connecting the scanner to the device (USB, or Bluetooth pairing in the OS) like any other keyboard.
  - The app only accepts codes matching Hugo's `HUGO-XXXXX` format, and ignores a repeat of the same code scanned again within 4 seconds — protects against double-stamping if a scanner re-fires a read (a trigger held a beat too long, or a scanner without its own reread-delay configured).
  - Worth setting anyway: most USB/Bluetooth scanners have a configurable "reread delay" (set via a config barcode in the scanner's manual) — setting that on the hardware itself, in addition to the app-side debounce above, gives the most reliable hands-free experience.
